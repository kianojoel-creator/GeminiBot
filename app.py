...
async def on_message(message: discord.Message):
    ...
    def make_multi_embed(fields: list, color: int = 0x3498DB) -> discord.Embed:
        """Erstellt ein Embed mit mehreren Übersetzungen in einem Kasten."""
        embed = discord.Embed(
            title=f"💬 • {author_name}",
            color=color
        )
    ...
    try:
        ...
        if lang == "DE":
            ...
            if fields:
                await message.reply(embed=make_multi_embed(fields), mention_author=False)
        ...
        # ── Fall 4: Andere Sprache (Gast) → DE + FR + PT in einem Embed ──
            ...
            if pt_text and pt_text.lower() != content.lower():
                fields.append(("🇧🇷 Português", pt_text))
            if fields:
                await message.reply(embed=make_multi_embed(fields, 0x9B59B6), mention_author=False)
