...
async def cmd_help(ctx):
    ...
    embed.add_field(
        ...
        value=(
            ...
            "`!translate status` – Status anzeigen / Voir le statut / Ver status\n"
            "`!ai [Text]` – KI fragen / Poser une question / Perguntar à IA\n"
            "`!übersetze` / `!traduire` – Bild übersetzen / Traduire une image / Traduzir imagem"
        ),
        inline=False
    )
    ...
    # ── Spieler ──
    embed.add_field(
        name="👥 Spieler-IDs / Joueurs  🔐 R5 • R4",
        value=(
            "`!spieler` / `!joueur` – Liste anzeigen / Afficher\n"
            "`!spieler add NAME ID` – Hinzufügen / Ajouter\n"
            "`!spieler delete NAME` – Löschen / Supprimer\n"
            "`!spieler suche NAME/ID` – Suchen / Rechercher"
        ),
        inline=False
    )
...
async def on_message(message: discord.Message):
    ...
    def make_guest_embed(de_text: str, fr_text: str, pt_text: str = "") -> discord.Embed:
        """Gast-Embed mit DE + FR + PT in einem Kasten."""
        embed = discord.Embed(
            title=f"🌍 • {author_name}",
        ...
        if pt_text:
            embed.add_field(name="🇧🇷 Português", value=pt_text, inline=False)
        embed.set_footer(text="VHA Translator", icon_url=LOGO_URL)
        return embed
    ...
    try:
        ...
        if lang == "DE":
            if reply_target_lang:
                # Antwort auf Gast → FR + PT + Gastsprache (kein DE nochmal)
                fr_text = await translate_text(content, "French")
                pt_text = await translate_text(content, "Brazilian Portuguese")
                ...
                if pt_text and pt_text.lower() != content.lower():
                    await message.reply(embed=make_embed("🇧🇷", pt_text, 0x3498DB), mention_author=False)
        ...
        # ── Fall 2: Französisch geschrieben ──────────────────────────────
            ...
            if reply_target_lang:
                # Antwort auf Gast → DE + PT + Gastsprache (kein FR nochmal)
                ...
                # Normaler französischer Text → DE + PT
                de_text = await translate_text(content, "German")
                pt_text = await translate_text(content, "Brazilian Portuguese")
                if de_text and de_text.lower() != content.lower():
                    await message.reply(embed=make_embed("🇩🇪", de_text, 0x3498DB), mention_author=False)
        ...
        # ── Fall 3: Portugiesisch geschrieben ────────────────────────────
        elif lang == "PT":
            if reply_target_lang:
                # Antwort auf Gast → DE + FR + Gastsprache (kein PT nochmal)
                de_text = await translate_text(content, "German")
                fr_text = await translate_text(content, "French")
                guest_text = await translate_text(content, LANG_NAMES.get(reply_target_lang, reply_target_lang))
                guest_flag = LANG_FLAGS.get(reply_target_lang, "🌐")
                ...
                # Normaler portugiesischer Text → DE + FR
                de_text = await translate_text(content, "German")
                fr_text = await translate_text(content, "French")
                if de_text and de_text.lower() != content.lower():
                    await message.reply(embed=make_embed("🇩🇪", de_text, 0x3498DB), mention_author=False)
        ...
        # ── Fall 4: Andere Sprache (Gast) → DE + FR + PT in einem Embed ──
        else:
            de_text = await translate_text(content, "German")
            fr_text = await translate_text(content, "French")
            pt_text = await translate_text(content, "Brazilian Portuguese")
            ...
            pt_ok = pt_text and pt_text.lower() != content.lower()
            if de_ok or fr_ok or pt_ok:
                await message.reply(
                    embed=make_guest_embed(
                        de_text if de_ok else "",
                        fr_text if fr_ok else "",
                        pt_text if pt_ok else ""
                    ),
